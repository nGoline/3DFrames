...
function printingGuide(result: BuildResult, config: FrameConfig, title: string): string {
  ...
  const lines = [
    ...
    'PRINTING',
    '--------',
    '  Before you  : these files carry geometry and no print settings, so they',
    '                slice with whatever profile is loaded. Check spiral vase',
    '                mode is off — a calibration profile often leaves it on, and',
    '                every part comes out as a single hollow wall.',
